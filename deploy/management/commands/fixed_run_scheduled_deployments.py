        ...
        for sched in pending:
            ...
            try:
                if sched.deploy_type == 'host' and sched.host:
                    ...
                    self.stdout.write(f'KEYS en extravars: {list(extravars.keys())}')
                    self.stdout.write(f'Valores críticos:')
                    self.stdout.write(f'- http_port: {extravars.get("http_port")}')
                    self.stdout.write(f'- https_port: {extravars.get("https_port")}')
                    self.stdout.write(f'- server_root: {extravars.get("server_root")}')
